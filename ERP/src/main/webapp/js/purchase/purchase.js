...
function deleteTbody(){
	...
	alert(k-2);
	$("input[name='deList["+(k-2)+"].puId']").remove();
	getTotalMoney(k);
	if(i>0){
		i--;
	}
...
function addTbody(){
	...
	$.ajax({
		...
		success:function(result){
			...
			var inputs="<input type='hidden' name='deList["+(k-2)+"].puId' />";
			$("#addDetailedForm").append(inputs);	
			var puId=$("input[name='puId']").val();
			$("input[name='deList["+(k-2)+"].puId']").val(puId);
		}
	});
	return page;
};
...
function queryMaxId(){
	$.ajax({
		...
		success:function(result){
			...
			$("input[name='puId']").val(result.puRealid);
		}
	});
